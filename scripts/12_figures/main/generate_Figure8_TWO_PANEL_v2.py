...
OUTDIR = ROOT / "results/session42_main_figure_editing/Figure8_SUBMISSION_v2"
...
PDF = FIGDIR / "Figure8_SUBMISSION_v2_cellular_seasonal_context.pdf"
PNG = FIGDIR / "Figure8_SUBMISSION_v2_cellular_seasonal_context.png"
SVG = FIGDIR / "Figure8_SUBMISSION_v2_cellular_seasonal_context.svg"
...
fig = plt.figure(figsize=(26.0, 9.2))
gs = fig.add_gridspec(
    1, 12,
    left=0.075,
    right=0.945,
...
# Former Figure 7 panels E/F become new Figure 8 panels A/B.
# Give each panel a large, publication-readable footprint.
axE = fig.add_subplot(gs[0, 0:6])
axF = fig.add_subplot(gs[0, 7:12])

# Invisible dummy axes allow the inherited frozen A-D computation
# blocks to run without changing any scientific calculations.
axA = fig.add_axes([0.001, 0.001, 0.001, 0.001])
axB = fig.add_axes([0.001, 0.001, 0.001, 0.001])
axC = fig.add_axes([0.001, 0.001, 0.001, 0.001])
axD = fig.add_axes([0.001, 0.001, 0.001, 0.001])
...
spine_x = 0.50
upper_branch_y = 0.695
lower_branch_y = 0.345

axA.plot([spine_x, spine_x], [0.72, lower_branch_y],
...
# Former Panel B architecture-class figure-level labels omitted:
# that panel is not part of the new Figure 8.

new_panel_titles = {
    "A": "Cell-state localization of the influenza program",
    "B": "Stable and context-dependent programs across seasons",
}

for label, axis in zip(["A", "B"], [axE, axF]):
    boxp = axis.get_position()
    ytitle = boxp.y1 + 0.030
    fig.text(
        boxp.x0 - 0.038, ytitle,
        label,
        fontsize=28.0,
        fontweight="bold",
        ha="left", va="bottom",
...
prov.to_csv(
    PROVDIR / "Figure8_SUBMISSION_v2_input_sha256.tsv",
    sep="\t", index=False,
)

# Save exact displayed top-15 ordering.
plotC.sort_values("session38_rank").to_csv(
    TABDIR / "Figure8_SUBMISSION_v2_legacy_panelC_top15.tsv",
    sep="\t", index=False,
)
plotD.to_csv(
    TABDIR / "Figure8_SUBMISSION_v2_legacy_panelD_bridge7.tsv",
    sep="\t", index=False,
)
E.to_csv(
    TABDIR / "Figure8_SUBMISSION_v2_panelA_localization.tsv",
    sep="\t", index=False,
)
plotF.to_csv(
    TABDIR / "Figure8_SUBMISSION_v2_panelB_stability.tsv",
    sep="\t", index=False,
)
...
# ============================================================
# FIGURE 8 v1 — TWO-PANEL READABILITY PASS
# Presentation only; frozen scientific content unchanged.
# ============================================================

# Hide all inherited non-rendered panels after their computations.
for _ax in [axA, axB, axC, axD]:
    _ax.set_visible(False)

# Panel A (former E): generous cell-state and axis typography.
for _t in axE.get_yticklabels():
    _t.set_fontsize(21.0)
    _t.set_fontweight("bold")
for _t in axE.get_xticklabels():
    _t.set_fontsize(18.5)
    _t.set_fontweight("bold")
axE.xaxis.label.set_fontsize(20.5)
axE.xaxis.label.set_fontweight("bold")
for _t in axE.texts:
    _t.set_fontsize(max(float(_t.get_fontsize()), 15.0))
    _t.set_fontweight("bold")
_legE = axE.get_legend()
if _legE is not None:
    for _t in _legE.get_texts():
        _t.set_fontsize(16.5)
        _t.set_fontweight("bold")
    _legE.set_bbox_to_anchor((0.50, -0.145))

# Panel B (former F): enlarge category labels, ticks, annotations and legend.
for _t in axF.get_yticklabels():
    _t.set_fontsize(21.0)
    _t.set_fontweight("bold")
for _t in axF.get_xticklabels():
    _t.set_fontsize(18.5)
    _t.set_fontweight("bold")
axF.xaxis.label.set_fontsize(20.5)
axF.xaxis.label.set_fontweight("bold")
for _t in axF.texts:
    _t.set_fontsize(max(float(_t.get_fontsize()), 15.0))
    _t.set_fontweight("bold")
_legF = axF.get_legend()
if _legF is not None:
    for _t in _legF.get_texts():
        _t.set_fontsize(16.5)
        _t.set_fontweight("bold")
    _legF.set_bbox_to_anchor((0.50, -0.145))


# Emphasize the principal cross-season takeaway.
for _t in axF.texts:
    if "150/170 genes (88.2%) persistent or predominant across seasons" in _t.get_text():
        _t.set_fontsize(16.5)
        _t.set_fontweight("bold")

# Preserve clean publication spines and adequate horizontal room for
# the right-side evaluable-count annotations.
axE.margins(y=0.10)
axF.margins(y=0.12)
axF.set_xlim(0, 112)
...
print("=== FIGURE 8 SUBMISSION v2 FINAL POLISH COMPLETE ===")
print("PDF:", PDF)
print("PNG:", PNG)
print("SVG:", SVG)
print()
...
print()
print("STATUS: FIGURE 8 SUBMISSION v2 FINAL-POLISH CANDIDATE")
